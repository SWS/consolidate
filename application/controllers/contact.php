...
class Contact extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('menu_model_ab', 'comman_model', 'menu_model', 'country_model'));
        $this->load->helper(array('assets', 'av_helper', 'cart_helper'));
        $this->load->language(array('header', 'contact', 'footer'));
        $this->check_lang();
    }
    ...
    public function index() {
        ...
        $data['contact_msg'] = $this->get_contact_msg(true);
    ...
    public function set_contact_form() {
        ...
        if ($this->input->post('operation')) {
            ...
            if (!empty($blocked_email) && isset($blocked_email[0]->dte_block) && $blocked_email[0]->dte_block != NULL && $blocked_email[0]->dte_block > $datelimit) {
                ...
                $where_param = array();
                if (isset($blocked_email[0]->str_email) && $blocked_email[0]->str_email != '') {
...
//
                    ...
                    if ($to == 'admin@kondar.ca,customerservice@kondar.ca') {
                        $this->email->set_header("To", 'KGT Customer Service Department <customerservice@kondar.ca>');
                    } else if ($to == 'maoui@kondar.ca,admin@kondar.ca,customerservice@kondar.ca') {
                        $this->email->set_header("To", 'KGT London Dubai <maoui@kondar.ca>');
                        $this->email->cc('KGT Customer Service Department <customerservice@kondar.ca>');
                    } else if ($to == 'rguez@kondar.ca,admin@kondar.ca,customerservice@kondar.ca') {
                        $this->email->set_header("To", 'KGT Dubai Branch <rguez@kondar.ca>');
                        $this->email->cc('KGT Customer Service Department <customerservice@kondar.ca>');
                    } else if ($to == 'benmohamed@kondar.ca,admin@kondar.ca,customerservice@kondar.ca') {
                        $this->email->set_header("To", 'KGT Tunis Branch <benmohamed@kondar.ca>');
                        $this->email->cc('KGT Customer Service Department <customerservice@kondar.ca>');
                    }
...
//edited by 4axiz to execute operation on block_email_list
    ...
    function get_contact_msg($check = false) {
        $select_param = "*";
        $result = $this->comman_model->get_timer('contact_message', $select_param);
        if ($check == true) {
            return $result;
        } else {
