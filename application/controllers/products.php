...
class Products extends CI_Controller {
    ...
    public function __construct() {
        ...
        parent::__construct();
        $this->load->model(array('product_model', 'menu_model', 'country_model', 'comman_model'));

        $this->load->helper(array('assets', 'av_helper', 'cart_helper', 'dbg_helper', 'common_helper'));
        $this->load->language(array('header', 'product', 'footer'));
        ...
        foreach ($data['vehicle_makers'] as $maker) {
            ...
            if (isset($maker_data1)) {
                foreach ($maker_data1 as $maker1) {
                    ...
                    if (!in_array($type_id, $data['vehicle_type_id']))
                        continue;
    ...
    function product_list($kgt_ref_number = '', $selected_vehicle_type_id = '', $selected_vehicle_maker_id = '') {
        ...
        $data['vehicle_category_ids'] = $this->session->userdata('vehicle_category_id');
        $data['vehicle_category_ids'] = array_unique($data['vehicle_category_ids'], SORT_REGULAR);
        $this->session->set_userdata(array('vehicle_category_id' => $data['vehicle_category_ids']));

        $data['vehicle_type_id'] = $this->session->userdata('vehicle_type_id');
        $data['vehicle_type_id'] = array_unique($data['vehicle_type_id'], SORT_REGULAR);
        ...
        $data['vehicle_brand_id'] = array_unique($data['vehicle_brand_id'], SORT_REGULAR);
        ...
        if ($productselected == false) { //1st parameter = ''
            ...
            //if all params in url
            if ($kgt_ref_number == 'select') { //  $selected_vehicle_type_id,  $selected_vehicle_maker_id
                $vehicle_category_ids = $this->product_model->get_product_category_by_type_id(array($selected_vehicle_type_id));
